...
app.post('/deleter', (req, res) => {
    ...
    MongoClient.connect("mongodb://localhost:27017/", {useNewUrlParser: true, useUnifiedTopology: true}, function(err, client) {
        if (err) throw err;
        console.log('\n\n\n\tConnected to MONGO. MODE - DELETE');
...
app.post('/editTask', (req, res) => {
    MongoClient.connect("mongodb://localhost:27017/", {useNewUrlParser: true, useUnifiedTopology: true}, function(err, client) {
        if (err) throw err;
        console.log('\n\n\n\tConnected to MONGO. MODE - EDIT');
        const db = client.db("ProjectTracker");
        db.collection("todos").updateOne({_id: ObjID(req.body.task._id)}, {$set: {
...
app.post('/todos', (req, res) => {
    ...
    MongoClient.connect("mongodb://localhost:27017/", { useNewUrlParser: true, useUnifiedTopology: true}, function(err, client) {
        if (err) throw err;
        console.log('\n\n\n\tConnected to MONGO. MODE - ADD --> ' + obj.toString());
        const db = client.db("ProjectTracker");
        db.collection("todos").insertOne(obj, function(err) {
            if (err) res.send(err);
            res.send("1 document inserted in collection");
